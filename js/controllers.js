...
app.controller("VidlistController", VidlistController);
...
function VidlistController($scope, Page) {
	Page.setTitle('Bilibili视频自动下载')
	Page.setIndex(111);
...
function PlayController($scope, Page, $routeParams) {
	Page.setTitle('视频播放页')
	Page.setIndex(222);

	var fileid = $routeParams.vid;
	//console.log(fileid);
	...
	const dp = new DPlayer({
		...
		screenshot: true,
		video: {
			url: '/video/'+fileid+'.flv',
			//url: '/video/50140829-87772133.flv',
			pic: 'demo.jpg',
		...
		danmaku: {
			id: fileid.split('-')[1],
			api: 'http://localhost:18080/danmaku/',
		},
	});
